...
import { LogOut, Calendar, Users, Home, MapPin, Bell } from 'lucide-react';
...
export default function DashboardLayout({ children }) {
  ...
  if (userData?.rol === 'parroco') {
    navItems.push({ name: 'Usuarios', path: '/usuarios', icon: <Users size={18} /> });
    navItems.push({ name: 'Capillas', path: '/capillas', icon: <MapPin size={18} /> });
  }

  return (
    <div className="flex h-screen" style={{ backgroundColor: 'hsl(var(--canvas))' }}>
      <aside className="glass" style={{ width: '280px', borderRight: '1px solid hsl(var(--border))', display: 'flex', flexDirection: 'column', zIndex: 20 }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Topbar */}
        <div style={{ height: '60px', borderBottom: '1px solid hsl(var(--border))', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 2rem' }}>
          <div style={{ position: 'relative' }}>
            ...
            >
              ...
              {notifications.length > 0 && (
                <span style={{ position: 'absolute', top: 0, right: 0, width: '8px', height: '8px', backgroundColor: 'hsl(var(--danger))', borderRadius: '50%' }}></span>
              )}
            </button>
            
            {showNotifications && (
              <div className="card glass animate-fade-in" style={{ position: 'absolute', top: '100%', right: 0, width: '300px', zIndex: 100, padding: '1rem', marginTop: '0.5rem' }}>
